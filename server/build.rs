    ...
    for result in csv_reader.records() {
        ...
        if record.get(5).unwrap() != "" {
            ...
            yt_id: \"{}\",
                ...
                html_escape::encode_text(record.get(0).unwrap()),
                html_escape::encode_text(record.get(2).unwrap()),
                record.get(1).unwrap(),
                record.get(6).unwrap_or("0"),
                html_escape::encode_text(record.get(3).unwrap()),
                html_escape::encode_text(record.get(4).unwrap()),
                id,
                my_yt_id,
            )
            .as_str();
