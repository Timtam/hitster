...
#[derive(Debug, Serialize, JsonSchema)]
pub struct UpdatePackError {
    pub message: String,
    #[serde(skip)]
    pub http_status_code: u16,
}

impl OpenApiResponderInner for UpdatePackError {
    fn responses(_generator: &mut OpenApiGenerator) -> Result<Responses, OpenApiError> {
        let mut responses = Map::new();
        responses.insert(
            "401".to_string(),
...
impl std::fmt::Display for UpdatePackError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "Update pack error `{}`", self.message,)
    }
}

impl std::error::Error for UpdatePackError {}

impl<'r> Responder<'r, 'static> for UpdatePackError {
    fn respond_to(self, _: &'r Request<'_>) -> response::Result<'static> {
        // Convert object to json
        let body = serde_json::to_string(&self).unwrap();
        Response::build()
...
#[derive(Debug, Serialize, JsonSchema)]
pub struct DeleteHitIssueError {
    pub message: String,
    #[serde(skip)]
    pub http_status_code: u16,
}

impl OpenApiResponderInner for DeleteHitIssueError {
    fn responses(_generator: &mut OpenApiGenerator) -> Result<Responses, OpenApiError> {
        let mut responses = Map::new();
        responses.insert(
            "401".to_string(),
            RefOr::Object(OpenApiResponse {
                description: "\
                # [401 Unauthorized](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/401)\n\
                This endpoint is only usable by an authenticated user who has delete permissions for issues.\
                "
                .to_string(),
                ..Default::default()
            }),
        ...
        responses.insert(
            ...
            RefOr::Object(OpenApiResponse {
                description: "\
                # [404 Not Found](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/404)\n\
                The issue with that ID doesn't exist for the selected hit.\
                "
                .to_string(),
                ..Default::default()
            }),
        ...
        responses.insert(
            ...
            RefOr::Object(OpenApiResponse {
                description: "\
                # [500 Internal Server Error](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/500)\n\
                The issue could not be deleted due to an internal error.\
                "
                .to_string(),
                ..Default::default()
            }),
...
impl std::fmt::Display for DeleteHitIssueError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "Delete hit issue error `{}`", self.message,)
    }
}

impl std::error::Error for DeleteHitIssueError {}

impl<'r> Responder<'r, 'static> for DeleteHitIssueError {
    fn respond_to(self, _: &'r Request<'_>) -> response::Result<'static> {
        // Convert object to json
        let body = serde_json::to_string(&self).unwrap();
        Response::build()
